...
void Entity::updateCollisions(){
	if (active && collideWithSolids && (moveSpeed > 0 || slideAmount > 0)){
		...
		for (auto entity = Entity::entities.begin(); entity != Entity::entities.end(); entity++){
			...
			if ((*entity)->active 
				//&& (*entity)->type != this->type //add this line back in if things of same type can walk through each other
				&& (*entity)->solid 
...
float Entity::SweptAABB(SDL_Rect movingBox, float vx, float vy, SDL_Rect otherBox, float &normalX, float &normalY){
	...
	//find time of collision and time of leacing for each axis (if statement is to prevent diving by zero)
	float xEntry, yEntry;
	float xExit, yExit;
	...
	if (entryTime > exitTime || xEntry < 0.0f && yEntry < 0.0f || xEntry > 1.0f || yEntry > 1.0f){
		...
		return 1.0f; //1 means moved full distance, no collisions
	}
	else
	{
		...
		//return the time of collision (0-1 e.g 0.234234)
		return entryTime;
	}
}
